...
import { SimulationTutorContext, TutorMessage, buildSimulationTutorPrompt } from '@/lib/simulation-tutor-context';

interface SimulationTutorProps {
  context: SimulationTutorContext;
}

const MAX_MESSAGES = 100;

export default function SimulationTutor({ context }: SimulationTutorProps) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const systemPrompt = buildSimulationTutorPrompt(context);
      const conversationHistory = messages.slice(-10).map(m => ({
        role: m.role,
        content: m.content
      }));

      // Call tutor-chat API (reuse existing endpoint)
      const response = await fetch('/api/tutor-chat', {
        ...
        body: JSON.stringify({
          message: userMessage,
          context: {
            courseTitle: context.simulationTitle,
            currentPageTitle: context.simulationTitle,
            currentPageType: 'chapter' as const,
            totalPages: 1
          },
            ...
            >
              ...
              <div className="relative bg-gradient-to-br from-violet-600 via-purple-600 to-fuchsia-600 p-4 flex items-center justify-between flex-shrink-0">
                ...
                <div className="flex items-center gap-3 relative z-10">
                  ...
                  <div>
                    <h3 className="font-bold text-white text-base tracking-tight">Lab Assistant</h3>
                  ...
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center py-8 max-w-[280px]">
                      ...
                      <p className="text-sm font-semibold text-gray-800 mb-2">
                        Hi! I'm your lab assistant 🔬
                      </p>
                      <p className="text-xs text-gray-500 leading-relaxed">
                        Ask me about <span className="font-semibold text-purple-600">{context.simulationTitle}</span>
                      </p>
                    </div>
                  </div>
                )}
